...
export async function processEmailAsync(
  ...
  console.log(`[VERSION] 1.0 Email Processing`);
  ...
  try {
    ...
    if (typeof paramOrTemplateId === 'object') {
      ...
      console.log(`[PARAMS] Lead ID: ${processLeadId || 'None'}`);
    ...
    console.log(`[VALIDATION] Validating required parameters`);
    if (!processTemplateId) {
      console.log(`[ERROR] Missing templateId in processEmailAsync`);
      addApiLog('Missing templateId in processEmailAsync', 'error', 'emails');
      throw new Error('Missing templateId');
    }

    if (!processSubmissionId) {
      console.log(`[ERROR] Missing submissionId in processEmailAsync`);
      addApiLog('Missing submissionId in processEmailAsync', 'error', 'emails');
      throw new Error('Missing submissionId');
    }

    if (!processRecipientEmail) {
      console.log(`[ERROR] Missing recipient email in processEmailAsync`);
      addApiLog('Missing recipient email in processEmailAsync', 'error', 'emails');
      throw new Error('Missing recipient email');
    }
    
    console.log(`[VALIDATION] Required parameters validated successfully`);
    ...
    try {
      ...
      console.log(`[DATABASE] Fetching email template: ${processTemplateId}`);
      addApiLog(`Fetching template: ${processTemplateId}`, 'info', 'emails');
      const template = await fetchTemplateById(processTemplateId);

      if (!template) {
        console.log(`[ERROR] Template not found: ${processTemplateId}`);
        addApiLog(`Template not found: ${processTemplateId}`, 'error', 'emails');
        throw new Error(`Template not found: ${processTemplateId}`);
      }
      
      console.log(`[DATABASE] Found template: ${template.name || 'Unnamed'} (ID: ${template.id})`);
      addApiLog(`Found template: ${template.name || 'Unnamed'} (ID: ${template.id})`, 'info', 'emails');
      
      // Fetch the submission to get additional data
      console.log(`[DATABASE] Fetching submission: ${processSubmissionId}`);
      addApiLog(`Fetching submission: ${processSubmissionId}`, 'info', 'emails');
      ...
      if (!submission) {
        console.log(`[ERROR] Submission not found: ${processSubmissionId}`);
        addApiLog(`Submission not found: ${processSubmissionId}`, 'error', 'emails');
        throw new Error(`Submission not found: ${processSubmissionId}`);
      }
      
      console.log(`[DATABASE] Found submission for form ID: ${submission.formId}`);
      addApiLog(`Found submission for form ID: ${submission.formId}`, 'info', 'emails');
