...
// Cache for form sections and fields to improve performance
...
const formCache: Record<string, FormCacheItem> = {};
...
export class BulkLeadSubmissionService {
  ...
  async processSubmission(formId: string, formData: Record<string, any>, sourceUrl?: string): Promise<SubmissionResult> {
    try {
      ...
      if (formCache[formId] && now - formCache[formId].timestamp < CACHE_TTL) {
        // Use cached form data
        console.log(`[LEAD SUBMISSION] Using cached form data for ${formId}`);
        form = formCache[formId].form;
        formSections = formCache[formId].sections;
      } else {
        // Cache miss or expired, fetch from database
        console.log(`[LEAD SUBMISSION] Cache miss for form ${formId}, fetching from database`);
        ...
        formSections = await prisma.formSection.findMany({
          where: { formId },
          select: {
            id: true,
            title: true,
        ...
        console.log(`[LEAD SUBMISSION] Cached form data for ${formId}`);
      }
      ...
      if (leadId) {
        ...
        // Format it to match the format used in async-variable-replacer2.ts
        // Format: lead-[formId-substring]-[random number]-[timestamp]
        const formIdPart = formId.substring(0, 8);
        const randomPart = Math.floor(Math.random() * 10000);
        const updatedTrackingToken = `lead-${formIdPart}-${randomPart}-${formSubmission.timeStamp}`;
